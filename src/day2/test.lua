...
function TestIdRange:test_find_invalid_ids_none()
...
function TestIdRange:test_find_invalid_ids_one()
...
function TestIdRange:test_find_invalid_ids_many()
...
function TestInvalidId:test_next_from_any_before_same_size()
    local id = day2.InvalidId.next_from_any(31)
...
function TestInvalidId:test_next_from_any_before_odd_digits()
    local id = day2.InvalidId.next_from_any(1)
    luaunit.assertEquals(id.value, 11)
    luaunit.assertEquals(id.value_str, "11")
    luaunit.assertEquals(id.repeated_half, 1)
    luaunit.assertEquals(id.num_digits, 2)

    local id = day2.InvalidId.next_from_any(919)
    ...
    local id = day2.InvalidId.next_from_any(1698522)
    luaunit.assertEquals(id.value, 10001000)
end

function TestInvalidId:test_next_from_any_is_invalid()
    local id = day2.InvalidId.next_from_any(3434)
...
function TestInvalidId:test_next_same_num_digits()
    local id = day2.InvalidId.new(33)
    local next = id:next()

    luaunit.assertItemsEquals(
        next,
        day2.InvalidId.new(44)
    )

    id = day2.InvalidId.new(5656)
    next = id:next()

    luaunit.assertItemsEquals(
        next,
        day2.InvalidId.new(5757)
    )
end

function TestInvalidId:test_next_more_digits()
    local id = day2.InvalidId.new(99)
    local next = id:next()

    luaunit.assertItemsEquals(
        next,
        day2.InvalidId.new(1010)
    )

    id = day2.InvalidId.new(9999)
    next = id:next()

    luaunit.assertItemsEquals(
        next,
        day2.InvalidId.new(100100)
    )
end
